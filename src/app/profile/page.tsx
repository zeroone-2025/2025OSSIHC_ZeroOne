...
export default function ProfilePage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-md px-4 pb-24 pt-8">
      ...
      <p className="mt-1 text-sm text-gray-500">식사 모드와 인원수는 LLM 질문에서 자동 수집합니다.</p>

      <section className="mt-6 rounded-3xl bg-white p-6 shadow">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">날씨 반영</h2>
            <p className="text-sm text-gray-500">비·강풍 시 가까운 식당을 우선합니다.</p>
          </div>
          <button
      ...
      <section className="mt-6">
        ...
        <p className="text-sm text-gray-500">선택한 알레르기 재료는 추천에서 제외합니다.</p>
            ...
            return (
              <button
                ...
                className={`rounded-2xl px-4 py-3 text-sm font-medium transition-all ${
                  active ? 'bg-red-500 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
                style={{ minHeight: 48 }}
              >
                {option.label}
      ...
      <section className="mt-8">
        ...
        <p className="text-sm text-gray-500">선택한 비선호 태그는 추천 시 감점합니다.</p>
        <div className="mt-3 grid grid-cols-2 gap-3">
